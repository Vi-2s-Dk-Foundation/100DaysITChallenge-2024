...
def get_playlist_videos(api_key, playlist_id, part="snippet"):
  ...
  Returns:
      A list of dictionaries containing video details, or an empty list if there's an error.
  """
...
df_plist.to_csv('play_data.csv', index=False)

# Create a Pandas DataFrame from the videos list
...
# Write the DataFrame to a csv file
df_videos.to_csv('all_videos_data.csv', index=False)
